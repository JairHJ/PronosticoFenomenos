...
export default function PredictionsHScreen() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://10.13.7.237:5000/pronos');
  ...
  return (
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          data={filteredData}
          keyExtractor={(item) => item.id_ph.toString()}
          renderItem={({ item }) => {
              ...
              <TouchableOpacity style={[styles.card, { backgroundColor: color }]}>
                <View style={styles.cardContent}>
                  <View style={styles.cardLeft}>
                    ...
                    <Text style={styles.details}>🌡️ {item.temp_ph} °C | 🌬️ {item.viento_ph} m/s | 🔽 {item.presion_ph} hPa</Text>
                    <Text style={styles.riskLevel}>⚠️ Nivel de Riesgo: {item.nom_dl}</Text>
                  </View>
